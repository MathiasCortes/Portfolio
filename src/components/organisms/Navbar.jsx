...
const Navbar = () => {
  ...
  const toggleList = sideMenuIcon ? (
    <div
        className ={`${isHidden ? 'activeToggle' : 'hiddenToggle'}`}
  ...
  const iconToggle = isHidden ? (
    ...
    <button className="relative flex items-center justify-center w-12 h-12 overflow-hidden transition duration-500 transform bg-white border rounded-sm hover:scale-95"
    onClick={handleClick}>
     
    <span className="absolute w-10 h-1 transition-transform duration-300 -rotate-45 bg-black rounded-sm">
    </span>
    <span className="absolute w-10 h-1 transition-transform duration-300 rotate-45 bg-black rounded-sm">
    </span>
    <span className="absolute w-10 h-1 transition-transform duration-300 translate-x-12 bg-black rounded-sm">
    </span>
    </button>
  ) : (
  
    <button className="relative flex items-center justify-center w-12 h-12 overflow-hidden transition duration-300 transform bg-white border rounded-sm hover:scale-95"
    onClick={handleClick}>
        <span className="transition-transform duration-300 -translate-y-3 lineToggle">
    </span>
    <span className="transition-transform duration-300 translate-y-3 lineToggle">
    </span>
    <span className="transition-transform duration-300 -translate-x-0 lineToggle">
    </span>
    </button>
  );
